...
## 炭蓝/铜绿五色试版（2026-09-23）

用户要求再试 charcoal-blue #264653、verdigris #2a9d8f、tuscan-sun #e9c46a、sandy-brown #f4a261、burnt-peach #e76f51。替代上一试版外围 token：炭蓝标题、铜绿选中与焦点、30% 铜绿混白边框、5% 铜绿混白工具栏、9% 暖黄混白页面底色。橙色登记备用，不强行加入常规装饰或科学证据；布局、字体及科学颜色规则不变。

验证：构建通过；P00533 1470×950 与 1230×837 无横向溢出，膜详情打开/Escape关闭正常；表格及序列的字体/颜色/背景抽查与原基线一致，无应用控制台错误。截图 /tmp/memvar-verdigris.png。Browser 实例为空，沿用已授权 Playwright；未做移动端或所有科研状态验收。

## 粉紫/冰蓝五色试版（2026-09-23）

用户指定 thistle #cdb4db、pastel-petal #ffc8dd、baby-pink #ffafcc、icy-blue #bde0fe、sky-blue #a2d2ff。外围使用14%粉紫混白页面、16%浅粉混白工具栏、48%粉紫混白边框、50%冰蓝混白选中背景，天蓝作为选中底线、婴儿粉少量用于文字操作hover。深色标题与焦点保留可读性，不用浅彩色做正文。科学色阶变量、数据表、序列与结构保持原样。

验证：构建通过；P00533 1230×837、1470×950 无横向溢出，膜详情打开/Escape关闭正常；抽查表格与序列字体/前景/背景与原基线一致，无应用控制台错误。截图 /tmp/memvar-pastel.png。Browser 无实例，沿用已授权 Playwright 后备；未验收移动端及全量状态。

## 搜索组合框修复与结构选区条上色（2026-09-23）
