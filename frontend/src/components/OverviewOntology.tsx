...
export function OverviewReactome({accession}:{accession:string}){
 ...
 return <section className="ov-section ov-pathways ov-wide">
  ...
  <div className="ov-section-body ov-pathway-compact">
   ...
   <Status loading={preview.isPending} error={preview.error} empty={!preview.data?.items.length}><ul className="ov-pathway-preview">{preview.data?.items.map((pathway,index)=><li key={String(pathway.association_id??index)}><GitBranch size={14}/><span>{display(pathway.name)}</span><ReactomeEvidenceLabel code={pathway.evidence_code}/></li>)}</ul></Status>
  </div>
